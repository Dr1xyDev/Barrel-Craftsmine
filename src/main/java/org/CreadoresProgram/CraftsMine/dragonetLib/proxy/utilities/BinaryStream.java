...
public class BinaryStream {
    ...
    public Item getSlot(){
        ...
        if(id <= 0){
            return new Item(0, 0, 0, new byte[0]);
        }
        ...
        return new Item(id, data, cnt, nbt);
    }
    
    public void putSlot(Item item){
        int id = (int) item.itemIns[0];
        if(item == null || id == 0){
            this.putShort(0);
            return;
        }
        this.putShort(id);
        int subcount = (int) item.itemIns[2];
        byte count = (byte) (subcount & 0xff);
        this.putByte(count);
        if(item.itemIns[1] == null){
            this.putShort(-1);
        }else{
            this.putShort((int) (item.itemIns[1]));
        }
        byte[] nbt = (byte[]) item.itemIns[3];
